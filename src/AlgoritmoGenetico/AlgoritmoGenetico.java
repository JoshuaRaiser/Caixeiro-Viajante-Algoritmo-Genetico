...
public class AlgoritmoGenetico {
    ...
    boolean mostrarEvolucao;
    float taxaMutacao;
    int numeroEvolucoes;

    public AlgoritmoGenetico(Mapa mapa, Configuracao configuracoes) {
        // constrói os cromossomos
        this.cromossomos = new int[configuracoes.getNumeroPopulacoes()][configuracoes.getNumeroCidades()];
        this.NUMERO_POPULACAO = configuracoes.getNumeroPopulacoes();
        this.NUMERO_CIDADES = configuracoes.getNumeroCidades();
        this.resultados = new int[configuracoes.getNumeroPopulacoes()];

        // configurações
        this.mostrarEvolucao = configuracoes.isMostrarEvolucao();
        this.taxaMutacao = configuracoes.getTaxaMutacao();
        this.numeroEvolucoes = configuracoes.getNumeroEvolucoes();
    ...
    public void executar() {
        ...
        for (i = 0; i < numeroEvolucoes; i++) {
            renovarCromossomos(cromossomos, resultados, taxaMutacao);
            calcularResultado(cromossomos, resultados, mapa);
            ordenar(cromossomos, resultados);
            if (mostrarEvolucao) {
    ...
    private void ordenar(int[][] cromossomos, int[] resultados) {
        // ordenando
        int i, i2;
        for (i = 0; i < 10; i++) {
            for (i2 = i; i2 < 10; i2++) {
